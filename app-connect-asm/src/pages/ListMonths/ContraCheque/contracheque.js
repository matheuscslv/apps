...
export default function ContraCheque(props) {
  ...
  async function componenetDidMount() {
    ...
    const cc = {
      year: String(props.navigation.getParam("year")),
      month: String(props.navigation.getParam("month")),
    };
  ...
  async function generatePDF() {
    ...
    const pdf = await connect_service.post(`/generate`, {
      contra: {
        periodo:
          String(props.navigation.getParam("month_text")) +
          '/' +
          String(props.navigation.getParam("year")),
        data,
      },
    });
                    ...
                    <View>
                      <View
                        ...
                        <Text
                          ...
                          Período:{' '}
                          {data?.discounts?.values[0]?.pivot.mes > 9
                            ? data?.discounts?.values[0]?.pivot.mes
                            : '0' + data?.discounts?.values[0]?.pivot.mes}
                  /{data?.discounts.values[0]?.pivot.ano}
                        </Text>
                      </View>
...
ContraCheque.navigationOptions = {
  header: null
}
